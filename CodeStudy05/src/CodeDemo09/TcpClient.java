 ...
 *      3.使用网络字节输出流OutputStream对象里的write，给服务器发送数据
 *      4.使用socket对象中的方法，getInputStream()获取网络字节输入流对象InputStream
 *      5.使用网络字节输入流对象InputStream对象中的方法read()读取服务器回写的数据
 *      6.释放资源(Socket)
 * 注意:
 *      1.客户端和服务器进行交互必须使用Socket中提供的网络流,不能使用自己创建的流对象
 *      2.当我们创建客户端Socket对象的时候，就会去请求服务器和服务器经过三次握手建立连接通路
 *          如果服务器没有启动，那么就会抛出异常 java.net.ConnectException: Connection refused: connect
 *          如果服务器已经启动，那么就可以进行交互
 */
